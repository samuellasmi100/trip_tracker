...
const BOND_USER = process.env.BOND_USER
const USER_BOND = process.env.USER_BOND
...
const initQueue = async () => {
    try {
        ...
        RabbitMQConnectionManager.listenToQueueMessages(RABBIT_MQ_INSTANCE_NAME,USER_BOND,handleMessageFromQueue)
    } catch (error) {
...
const returnMessageToClient = async (msg) => {
 ...
 const sendMessageToQueue = async (data) => {
    try {
        RabbitMQConnectionManager.pushMessageToQueue(RABBIT_MQ_INSTANCE_NAME, BOND_USER, JSON.stringify(data))
    } catch (error) {
      console.log(`failed to init api_key queue, error: ${error}`)
    }
  }
