...
const assignMainRoom = async (roomDetails, familyId) => {
    const checkIfUserAlreadyAssign = await userRoomsDb.getFamilyRoom(familyId)
...
const getFamilyRoom = async (id) => {
    const familyRooms = await userRoomsDb.getFamilyRoom(id) 
    const userAssignRoom = await userRoomsDb.getAllUserRooms(id)
